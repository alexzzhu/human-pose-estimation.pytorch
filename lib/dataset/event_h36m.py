...
class EventH36MDataset(EventJointsDataset):
    ...
    # Original joints to new joints allocation:
    def _get_db(self):
        #joint_assignment_14 = [0, 1, 2, 3, 4, 5, None, None, 12, 13, 6, 7, 8, 9, 10, 11]
        ...
        n_ima = len(anno['views'])
        gt_db = []

        indices = anno['views'][:, 2]
        image_names = anno['imgname'][indices]
        centers = anno['center'][indices]
        scales = anno['scale'][indices]
        parts = anno['part'][indices]
        
        for i in range(n_ima):
            image_file = os.path.join(self.root, 'sequences',
                                      image_names[i].decode('UTF-8'))

            c = np.array(centers[i], dtype=np.float)
            s = np.array([scales[i], scales[i]], dtype=np.float)
            ...
            if self.image_set != 'test':
                ...
                joints_vis = np.ones((self.num_joints,)) #np.array(anno['visible'][i])
